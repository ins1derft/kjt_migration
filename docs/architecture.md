...
- Next.js 16, App Router, SSR. Стилизация — TailwindCSS + shadcn/ui primitives (`button`, `card`, `input`, `textarea`, `badge`), utility `cn` в `src/lib/utils.ts`. Дизайн‑система обновлена под новый лендинг: брендовое ядро (Open Sans + локальный Lorin, палитра brand.sky/orange/gradient start-mid-end), `shadow-glow`, `bg-brand-gradient` и новые анимации/базовые токены в `app/globals.css` и `tailwind.config.js` (legacy container-shell/section-shell убраны, секции верстаются явными `max-w-6xl`/`px-4 xl:px-12` отступами).
- Главные маршруты:
- `/` (`app/page.tsx`) — новый маркетинговый лендинг из дизайн-системы `src/design/*` (Hero c 3D-слайдером YouTube, HeroContent, values FeatureGrid, ProductCarousel, TrustedBy, Stats с анимацией счётчиков, Core Features, WhyUs, ShowroomCTA, GamesGallery с marquee, Testimonials, News, TransformCTA). Для ProductCarousel используется `/api/products` (slug/name/slogan/subtitle/hero_image), для GamesGallery — `/api/games`; вспомогательные копирайты и тексты лежат в `src/design/data.ts`.
...
| `Product` | `products` | `slug` unique, `name`, `slogan`, `subtitle`, `excerpt`, `description`, `hero_image`, `product_type`, `default_cta_label` | `variants` hasMany (ordered by `position`), `industries` m2m, `games` m2m |
