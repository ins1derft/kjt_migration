...
- Блоковый рендерер: типы → `src/lib/blocks/types.ts` (перенесены из прежнего design), реестр `src/lib/blocks/registry.tsx` рендерит блоки новым набором дизайн-компонентов (Hero, HeroContent, FeatureGrid, ProductCarousel, GamesGallery, News, Stats, FAQ, WhyUs, ProductDescription, ProductSpecs, CompareModels, CTASection, HighlightCTA, Reviews, TrustedBy) из `src/components/blocks`. Легаси-блоки на shadcn/ui перемещены в `src/components/blocks/_legacy` и исключены из сборки. Формат данных API прежний `{ name, key, values }`, маппинг на новый дизайн идёт по `name`. Каждый layout в MoonShine теперь имеет опциональные `padding.top`/`padding.bottom` (none/xs/sm/md/lg/xl/2xl) для кастомизации вертикальных отступов секции; по умолчанию используются прежние значения из верстки.
...
- **Отзывы:** `ReviewResource` (name, review_date, rating 1–5, текст TinyMCE, аватар upload в `storage/app/public/reviews`, source_url, position, active).
